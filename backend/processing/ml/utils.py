...
# Output folders for intermediate results
...
create_folders([
    ...
    HPE_FRAMES_FOLDER,
    HPE_SEG_FRAMES_FOLDER,
    DETECTION_FRAMES_FOLDER
])

# ----------------- Main Image Processing Logic ------------------
async def process_images(input_folder: Path, mode: str, output_folder: Path):
    """
    Process a folder of images based on the selected mode.
    Modes:
      - "Fragmentation"
      - "Segmentation"
      - "HPE"
    ...
    try:
        if mode == "Fragmentation":
            ...
            clear_folders([SEGMENTED_FRAMES_FOLDER, HPE_FRAMES_FOLDER, HPE_SEG_FRAMES_FOLDER])
            seg_task = asyncio.create_task(segment_images(input_folder=input_folder, output_folder=SEGMENTED_FRAMES_FOLDER))
            hpe_task = asyncio.create_task(hpe_images(input_folder=input_folder, output_folder=HPE_FRAMES_FOLDER))
            await asyncio.gather(seg_task, hpe_task)
            hpe_results = hpe_task.result()
            project_hpe_onto(
                hpe_results=hpe_results,
                segmented_folder=SEGMENTED_FRAMES_FOLDER,
                project_onto_folder=SEGMENTED_FRAMES_FOLDER,
                output_folder=HPE_SEG_FRAMES_FOLDER
            )
            ...
            project_hpe_onto(
                hpe_results=hpe_results,
                segmented_folder=SEGMENTED_FRAMES_FOLDER,
                project_onto_folder=input_folder,
                output_folder=DETECTION_FRAMES_FOLDER
            )
        ...
        print(f"[ERROR] Processing failed in mode: {mode}. Error: {e}")
        raise
